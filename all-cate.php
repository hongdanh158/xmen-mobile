<?php include('header.php') ?>
	<div class="all-cate">
		<div class="cate">
			<h3 class="all-cate-title">DẦU GỘI</h3>
	  		<div class="all-product shampoo">
	  			<div class="item luxury">
	  				<a href="dau-goi-nuoc-hoa.php" class="images">
	  					<img src="images/cate-detail/dau-goi-650/luxury.png" alt="" class="thumbnail">
	  				</a>
	  			</div>
	  			<div class="item intense">
	  				<a href="dau-goi-nuoc-hoa.php" class="images">
	  					<img src="images/cate-detail/dau-goi-650/intense.png" alt="" class="thumbnail">
	  				</a>
	  			</div>
	  			<div class="item motion active">
	  				<a href="dau-goi-nuoc-hoa.php" class="images">
	  					<img src="images/cate-detail/dau-goi-650/motion.png" alt="" class="thumbnail">
	  				</a>
	  			</div>
	  		</div>
	  	</div>
	  	<div class="cate">
			<h3 class="all-cate-title">SỮA TẮM</h3>
	  		<div class="all-product shampoo">
	  			<div class="item luxury">
	  				<a href="sua-tam-nuoc-hoa.php" class="images">
	  					<img src="images/cate-detail/dau-goi-650/luxury.png" alt="" class="thumbnail">
	  				</a>
	  			</div>
	  			<div class="item intense">
	  				<a href="sua-tam-nuoc-hoa.php" class="images">
	  					<img src="images/cate-detail/dau-goi-650/intense.png" alt="" class="thumbnail">
	  				</a>
	  			</div>
	  			<div class="item motion active">
	  				<a href="sua-tam-nuoc-hoa.php" class="images">
	  					<img src="images/cate-detail/dau-goi-650/motion.png" alt="" class="thumbnail">
	  				</a>
	  			</div>
	  		</div>
	  	</div>
	  	<div class="cate">
			<h3 class="all-cate-title">NƯỚC HOA</h3>
	  		<div class="all-product perfume">
	  			<div class="item luxury">
	  				<a href="nuoc-hoa.php" class="images">
	  					<img src="images/cate-detail/nuoc-hoa/luxury.png" alt="" class="thumbnail">
	  				</a>
	  			</div>
	  			<div class="item intense">
	  				<a href="nuoc-hoa.php" class="images">
	  					<img src="images/cate-detail/nuoc-hoa/intense.png" alt="" class="thumbnail">
	  				</a>
	  			</div>
	  			<div class="item motion active">
	  				<a href="nuoc-hoa.php" class="images">
	  					<img src="images/cate-detail/nuoc-hoa/motion.png" alt="" class="thumbnail">
	  				</a>
	  			</div>
	  		</div>
	  	</div>
		<div class="cate">
			<h3 class="all-cate-title">XỊT TOÀN THÂN</h3>
	  		<div class="all-product spray">
	  			<div class="item luxury">
	  				<a href="xit-toan-than.php" class="images">
	  					<img src="images/cate-detail/xit-toan-than/luxury.png" alt="" class="thumbnail">
	  				</a>
	  			</div>
	  			<div class="item intense">
	  				<a href="xit-toan-than.php" class="images">
	  					<img src="images/cate-detail/xit-toan-than/intense.png" alt="" class="thumbnail">
	  				</a>
	  			</div>
	  			<div class="item motion active">
	  				<a href="xit-toan-than.php" class="images">
	  					<img src="images/cate-detail/xit-toan-than/motion.png" alt="" class="thumbnail">
	  				</a>
	  			</div>
	  		</div>
	  	</div>
		<div class="cate">
			<h3 class="all-cate-title">LĂN KHỬ MÙI</h3>
	  		<div class="all-product roller">
	  			<div class="item luxury">
	  				<a href="lan-khu-mui.php" class="images">
	  					<img src="images/cate-detail/lan-khu-mui/luxury.png" alt="" class="thumbnail">
	  				</a>
	  			</div>
	  			<div class="item intense">
	  				<a href="lan-khu-mui.php" class="images">
	  					<img src="images/cate-detail/lan-khu-mui/intense.png" alt="" class="thumbnail">
	  				</a>
	  			</div>
	  			<div class="item motion active">
	  				<a href="lan-khu-mui.php" class="images">
	  					<img src="images/cate-detail/lan-khu-mui/motion.png" alt="" class="thumbnail">
	  				</a>
	  			</div>
	  		</div>
	  	</div>
		<div class="cate">
			<h3 class="all-cate-title">TẠO KIỂU TÓC</h3>
	  		<div class="all-product wax">
	  			<div class="item luxury">
	  				<a href="san-pham-tao-kieu-toc.php" class="images">
	  					<img src="images/cate-detail/wax/luxury.png" alt="" class="thumbnail">
	  				</a>
	  			</div>
	  			<div class="item intense">
	  				<a href="san-pham-tao-kieu-toc.php" class="images">
	  					<img src="images/cate-detail/wax/intense.png" alt="" class="thumbnail">
	  				</a>
	  			</div>
	  			<div class="item motion active">
	  				<a href="san-pham-tao-kieu-toc.php" class="images">
	  					<img src="images/cate-detail/wax/motion.png" alt="" class="thumbnail">
	  				</a>
	  			</div>
	  		</div>
	  	</div>
	</div>
<?php include('footer.php') ?>
</body>
</html>
